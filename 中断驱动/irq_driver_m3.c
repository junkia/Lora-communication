...
static int __init irq_driver_init(void)
{
    ...
    if (irqdev.major) {
        ...
        ret = register_chrdev_region(dev, 1, "loram3_irq_driver");
    } else {
        ret = alloc_chrdev_region(&dev, 0, 1, "loram3_irq_driver");
    ...
    /* 3、创建类 */
    irqdev.class = class_create(THIS_MODULE, "loram3_irq_driver");
    ...
    /* 4、创建设备 */
    irqdev.device = device_create(irqdev.class, NULL, dev, NULL, "loram3_irq_driver");
    ...
    /* 5、初始化 GPIO 和中断 */
    irqdev.nd = of_find_node_by_name(NULL, "loram3"); // 修改为 loram4 或 loram3
    if (!irqdev.nd) {
        printk(KERN_ERR "Failed to find loram4/loram3 node\n");
        return -ENODEV;
    }
    ret = of_get_named_gpio(irqdev.nd, "dio1-gpio", 0);
    if (ret < 0) {
    ...
    ret = gpio_request(irqdev.irq_gpio, "irq_gpio");
    if (ret < 0) {
    ...
    ret = gpio_direction_input(irqdev.irq_gpio);
    if (ret < 0) {
    ...
    ret = request_irq(gpio_to_irq(irqdev.irq_gpio), my_interrupt_handler, IRQF_TRIGGER_RISING, "loram3_irq_driver_dio1",&irqdev);
    if (ret < 0) {
    ...
    printk(KERN_INFO "loram3_irq_driver initialized\n");
    return 0;
}
...
static void __exit irq_driver_exit(void)
{
    ...
    printk(KERN_INFO "loram3_irq_driver exited\n");
}
